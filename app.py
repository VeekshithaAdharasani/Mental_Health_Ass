...
if "emotional_window" not in st.session_state:
    st.session_state.emotional_window = []
if "support_mode" not in st.session_state:
    st.session_state.support_mode = "Normal Conversation"
if "emotional_confidence" not in st.session_state:
    st.session_state.emotional_confidence = 0.0
if "elevated_state_until" not in st.session_state:
    st.session_state.elevated_state_until = 0.0
if "distress_pattern_count" not in st.session_state:
    st.session_state.distress_pattern_count = 0
if "stabilization_failures" not in st.session_state:
    st.session_state.stabilization_failures = 0
if "show_grounding_modal" not in st.session_state:
    st.session_state.show_grounding_modal = False
if "grounding_modal_id" not in st.session_state:
    st.session_state.grounding_modal_id = ""
if "last_grounding_at" not in st.session_state:
    st.session_state.last_grounding_at = 0.0
if "grounding_modal_expires_at" not in st.session_state:
    st.session_state.grounding_modal_expires_at = 0.0
...
def close_expired_breathing_popup():
    ...
    if st.session_state.get("show_breathing_popup") and expires_at and time.time() >= expires_at:
        ...
        st.session_state.breathing_popup_started_at = 0.0
        st.session_state.breathing_popup_expires_at = 0.0
    grounding_expires_at = st.session_state.get("grounding_modal_expires_at", 0.0)
    if st.session_state.get("show_grounding_modal") and grounding_expires_at and time.time() >= grounding_expires_at:
        st.session_state.show_grounding_modal = False
        st.session_state.grounding_modal_expires_at = 0.0
...
def score_emotional_message(text):
    """Lightweight score for rolling trend analysis."""
    q = normalize_query_text(text)
    score = 0
    categories = set()
    patterns = {
        "crisis": (SELF_HARM_KEYWORDS, 9),
        "breathing": (["can't breathe", "cannot breathe", "cant breathe", "no air", "suffocat", "hyperventilat"], 6),
        "panic": (["panic", "panicking", "shaking", "shivering", "trembling", "heart racing", "dizzy", "faint", "chest"], 4),
        "hopelessness": (["hopeless", "helpless", "can't go on", "cant go on", "worthless", "burden"], 3),
        "anxiety": (["anxious", "scared", "afraid", "worried", "overwhelmed", "stressed"], 2),
        "recovery": (RECOVERY_PHRASES, -4),
    }
    for category, (phrases, weight) in patterns.items():
        if any(phrase in q for phrase in phrases):
            score += weight
            categories.add(category)
    if any(phrase in q for phrase in ESCALATION_INDICATORS):
        score += 4
        categories.add("escalation")
    return score, categories

def get_support_mode(severity, emotion=None, analysis=None):
    """Map analysis into the six internal support modes."""
    analysis = analysis or {}
    trend = analysis.get("escalation_trend", st.session_state.get("escalation_trend", "stable"))
    if severity >= 5 or emotion == "crisis":
        return "Crisis Support"
    if severity >= 4:
        return "Panic Stabilization"
    if severity >= 3 or analysis.get("physical_symptoms"):
        return "Anxiety Support"
    if st.session_state.get("recovery_streak", 0) >= 1 or trend == "improving":
        return "Recovery Mode"
    if severity >= 1 or emotion in {"sadness", "stress", "burnout", "future_anxiety", "trauma"}:
        return "Emotional Support"
    return "Normal Conversation"

def get_mode_guidance(mode):
    guidance = {
        "Normal Conversation": "natural, reflective, conversational",
        "Emotional Support": "warm, validating, 2 to 4 short paragraphs",
        "Anxiety Support": "concrete, grounding-oriented, low pressure",
        "Panic Stabilization": "very short, step-by-step, no long explanations",
        "Crisis Support": "clear, supportive, direct about real-world help",
        "Recovery Mode": "gentle reinforcement, calm and hopeful",
    }
    return guidance.get(mode, guidance["Normal Conversation"])

def should_show_breathing_popup(answer, analysis):
    """Show breathing UI only when the assistant intentionally used a breathing intervention."""
    if not detect_breathing_trigger(answer):
        return False
    strategy = st.session_state.get("last_response_strategy")
    if strategy not in {"breathing", "grounding_breath"}:
        return False
    if analysis.get("severity", 0) < 2 and not analysis.get("physical_symptoms"):
        return False
    return time.time() - st.session_state.get("last_breathing_popup_at", 0.0) > 90

def open_grounding_modal():
    now = time.time()
    st.session_state.show_grounding_modal = True
    st.session_state.grounding_modal_id = f"grounding-{int(now * 1000)}-{random.randint(1000, 9999)}"
    st.session_state.grounding_modal_expires_at = now + 24
    st.session_state.last_grounding_at = now
...
def analyze_severity(query, history_messages, severity_history=None):
    ...
    rolling_score = 0.0
    rolling_categories = []
    weighted_recent = previous_user_messages[-5:] + [q]
    for index, message in enumerate(weighted_recent):
        message_score, categories = score_emotional_message(message)
        recency_weight = 0.58 + ((index + 1) / max(1, len(weighted_recent))) * 0.42
        rolling_score += message_score * recency_weight
        rolling_categories.extend(categories)
    repeated_patterns = {
        category: rolling_categories.count(category)
        for category in set(rolling_categories)
        if rolling_categories.count(category) >= 2
    }
    if repeated_patterns:
        pattern_bonus = min(6, sum(repeated_patterns.values()))
        score += pattern_bonus
        detected_symptoms.append("pattern: repeated distress")
    ...
    if rolling_score >= 8 and score >= 1:
        score += 2
        detected_symptoms.append("window: elevated recent distress")
    ...
    recovery_signal = any(phrase in q for phrase in RECOVERY_PHRASES)
    previous_severity = severity_history[-1] if severity_history else 0
    if severity < previous_severity and previous_severity >= 3 and not recovery_signal:
        severity = max(severity, previous_severity - 1)
        risk_level = "high" if severity >= 3 else risk_level
        detected_symptoms.append("continuity: elevated state retained")
    elif recovery_signal and previous_severity >= 2:
        severity = max(1, min(severity, previous_severity - 1))
        risk_level = "easing"
        detected_symptoms.append("recovery: gradual downshift")

    confidence_base = min(0.95, 0.28 + (min(abs(score), 12) / 18))
    if repeated_patterns:
        confidence_base += 0.08
    if severity_history:
        confidence_base += min(0.08, len(severity_history[-4:]) * 0.02)
    emotional_confidence = max(0.05, min(0.98, confidence_base))
    ...
    if len(simulated_history) >= 3:
        ...
        if last_three[-1] > last_three[0] or (rolling_score >= 10 and not recovery_signal):
    ...
    return {
        ...
        "emotional_progression": escalation_trend,
...
def update_severity_state(analysis):
    ...
    window = st.session_state.get("emotional_window", [])
    window.append({
        "time": now,
        "severity": severity,
        "score": analysis.get("score", 0),
        "rolling_score": analysis.get("rolling_score", 0),
        "patterns": analysis.get("repeated_patterns", {}),
        "trend": analysis.get("escalation_trend", "stable"),
    })
    st.session_state.emotional_window = window[-8:]
    ...
    st.session_state.emotional_confidence = analysis.get("emotional_confidence", 0.0)
    st.session_state.distress_pattern_count = sum(analysis.get("repeated_patterns", {}).values())
    ...
    st.session_state.support_mode = get_support_mode(severity, None, analysis)
    if severity >= 3:
        st.session_state.elevated_state_until = now + 120
    if severity >= 2:
...
def choose_panic_strategy(severity):
    ...
    failures = st.session_state.get("stabilization_failures", 0)
    if severity >= 5 or turns >= 5 or failures >= 3:
        return "trusted_help"
    if not has_used_strategy("emotional_support"):
        return "emotional_support"
    if not has_used_strategy("breathing") and failures == 0:
        return "breathing"
...
def get_adaptive_panic_response(query, severity, analysis=None):
    ...
    if strategy == "grounding":
        if has_used_strategy("breathing") or st.session_state.get("breathing_attempts", 0) > 0:
            lead = "We already tried slowing the breathing, and it still seems intense, so let's change the approach.\n\n"
        else:
            lead = "Let's ground through the room instead of forcing calm.\n\n"
        return (
            lead +
...
def generate_llm_response(query, context, intent, emotion, severity):
    ...
    if severity >= 2 or any(sign in query.lower() for sign in failure_signs):
        ...
        if any(sign in query.lower() for sign in failure_signs):
            st.session_state.stabilization_failures += 1
    else:
        st.session_state.repeated_distress = 0
    ...
    if any(phrase in query.lower() for phrase in RECOVERY_PHRASES):
        ...
        st.session_state.repeated_distress = 0
        st.session_state.stabilization_failures = max(0, st.session_state.stabilization_failures - 1)
    
    if emotion in {"panic", "anxiety", "stress"}:
        st.session_state.last_emotion = emotion

    # 🚨 CRITICAL ESCALATION FIRST
    if "ground me" in query.lower() or "5-4-3-2-1" in query.lower() or "54321" in query.lower():
        remember_support_strategy("grounding_breath")
        return (
            "Yes. Keep your eyes open and let the room become your anchor.\n\n"
            "Name 5 things you can see, 4 things you can feel, 3 sounds you hear, 2 body sensations, and 1 slow exhale.\n\n"
            "No need to do it perfectly. Just move one step at a time."
        )
    ...
    support_mode = st.session_state.get("support_mode", "Normal Conversation")
    system_prompt = f"""
...
- If breathing or grounding was already attempted, acknowledge that and shift methods
...
{support_mode} - {get_mode_guidance(support_mode)}

Recent support strategies:
{used_strategies}

Stabilization failures:
{st.session_state.get("stabilization_failures", 0)}
"""
...
if st.session_state.get("severity", 0) >= 3:
    st.markdown("""
    <style>
    .stApp {
        animation: stabilizationPulse 7s ease-in-out infinite;
    }
    .header-section {
        box-shadow: 0 24px 70px rgba(0, 0, 0, 0.42), 0 0 70px rgba(94, 234, 212, 0.10) !important;
    }
    .message-bubble-user {
        box-shadow: 0 10px 30px rgba(37, 99, 235, 0.18) !important;
    }
    @keyframes stabilizationPulse {
        0%, 100% { filter: saturate(0.94) brightness(0.98); }
        50% { filter: saturate(0.88) brightness(1.02); }
    }
    </style>
    """, unsafe_allow_html=True)
...
if query:
    ...
    st.session_state.support_mode = get_support_mode(severity_analysis["severity"], emotion, severity_analysis)
    ...
    # Show breathing support only for intentional stabilization, not any mention of breathing.
    if should_show_breathing_popup(answer, severity_analysis):
        start_breathing_popup()
        st.session_state.last_breathing_popup_at = time.time()
    else:
        if not st.session_state.get("show_grounding_modal", False):
            st.session_state.show_breathing_popup = False
            st.session_state.breathing_popup_active = False
...
if st.session_state.show_grounding_modal:
    grounding_id = st.session_state.get("grounding_modal_id", "grounding-modal")
    grounding_expires_at = st.session_state.get("grounding_modal_expires_at", 0.0)
    grounding_remaining_ms = max(500, int((grounding_expires_at - time.time()) * 1000))
    grounding_html = f"""
    <script>
    (function() {{
        const modalId = "{grounding_id}";
        const durationMs = {grounding_remaining_ms};
        const doc = window.parent.document;
        const storageKey = "groundingModalDismissed:" + modalId;

        function closeGrounding() {{
            const current = doc.getElementById("grounding-modal-root");
            if (!current) return;
            current.classList.add("grounding-modal-closing");
            window.parent.localStorage.setItem(storageKey, "1");
            setTimeout(() => current.remove(), 360);
        }}

        if (window.parent.localStorage.getItem(storageKey) === "1") return;
        const old = doc.getElementById("grounding-modal-root");
        if (old) old.remove();

        let style = doc.getElementById("grounding-modal-style");
        if (!style) {{
            style = doc.createElement("style");
            style.id = "grounding-modal-style";
            style.textContent = `
                #grounding-modal-root {{
                    position: fixed;
                    inset: 0;
                    z-index: 2147482999;
                    display: flex;
                    ...
                    animation: groundingIn 320ms ease-out;
                }}
                #grounding-modal-root.grounding-modal-closing {{
                    animation: groundingOut 320ms ease-in forwards;
                }}
                .grounding-card {{
                    ...
                    background:
                        ...
                        radial-gradient(circle at 50% 0%, rgba(94, 234, 212, 0.13), transparent 56%);
                    border: 1px solid rgba(125, 211, 252, 0.28);
                    box-shadow: 0 28px 80px rgba(0, 0, 0, 0.46), 0 0 80px rgba(94, 234, 212, 0.12);
                    position: relative;
                }}
                .grounding-close {{
                    position: absolute;
                ...
                .grounding-step {{
                    ...
                    font-size: 13.5px;
                    line-height: 1.4;
                    animation: groundingStep 9s ease-in-out infinite;
                }}
                .grounding-step:nth-child(2) {{ animation-delay: 1.2s; }}
                .grounding-step:nth-child(3) {{ animation-delay: 2.4s; }}
                .grounding-step:nth-child(4) {{ animation-delay: 3.6s; }}
                .grounding-step:nth-child(5) {{ animation-delay: 4.8s; }}
                @keyframes groundingIn {{ from {{ opacity: 0; }} to {{ opacity: 1; }} }}
                @keyframes groundingOut {{ from {{ opacity: 1; }} to {{ opacity: 0; }} }}
                @keyframes groundingStep {{
                    0%, 100% {{ border-color: rgba(148, 163, 184, 0.14); background: rgba(15, 23, 42, 0.56); }}
                    50% {{ border-color: rgba(94, 234, 212, 0.34); background: rgba(14, 116, 144, 0.16); }}
                }}
            `;
            doc.head.appendChild(style);
        }}
        ...
        const root = doc.createElement("div");
        root.id = "grounding-modal-root";
        root.setAttribute("role", "dialog");
        root.setAttribute("aria-modal", "true");
        root.innerHTML = `
            <div class="grounding-card">
                <button class="grounding-close" type="button" aria-label="Close grounding exercise">&times;</button>
                <div class="grounding-title">Ground Me</div>
                <div class="grounding-copy">Keep your eyes open. Let the room do some of the work for you.</div>
                <div class="grounding-steps">
                    <div class="grounding-step">5 things you can see</div>
                    <div class="grounding-step">4 things you can feel or touch</div>
                    <div class="grounding-step">3 sounds you can hear</div>
                    <div class="grounding-step">2 slow body sensations: feet, hands, shoulders</div>
                    <div class="grounding-step">1 gentle exhale, no forcing</div>
                </div>
            </div>
        `;
        root.addEventListener("click", function(event) {{
            if (event.target === root || event.target.closest(".grounding-close")) closeGrounding();
        }});
        doc.body.appendChild(root);
        window.setTimeout(closeGrounding, durationMs);
    }})();
    </script>
    """
    components.html(grounding_html, height=0, width=0)
...
with st.sidebar:
    ...
    if st.session_state.chat_history:
        ...
        if last_query:
            ...
            confidence = int(st.session_state.get("emotional_confidence", 0.0) * 100)
            support_mode = st.session_state.get("support_mode", get_support_mode(severity, emotion, severity_analysis))
            ...
            <div class="analysis-card">
                ...
                <span class="analysis-pill">Confidence {confidence}%</span>
            </div>
            <div class="analysis-card">
                <div class="analysis-kicker">Support Mode</div>
                <div class="analysis-value">{support_mode}</div>
                <div class="analysis-muted">{support_stage}</div>
                ...
                <span class="analysis-pill">Patterns {st.session_state.get("distress_pattern_count", 0)}</span>
            </div>
            <div class="analysis-card">
